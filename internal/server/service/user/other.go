...
func GetRandStr(min int, max int) string {
	var len = base2.RandInt(min, max)
	var i = 0
	var str = ""
	for i = 0; i < len; i++ {
		str += randChats[base2.RandInt(0, randChatsSize*3)%randChatsSize]
...
func GetRanUserName() string {
	str := firstName[base2.RandInt(0, firstNameLen*3)%firstNameLen]
	lSize := base2.RandInt(0, 6)%2 + 1
	for i := 0; i < lSize; i++ { //随机产生2位或者3位的名
		str += lastName[base2.RandInt(0, lastNameLen*3+i)%lastNameLen]
	}
	return str
}

type testTotalBatchInsertResult struct {
	successCount int
	errorCount   int
	startTime    int64
}

func TestTotalBatchInsert(ctrip int, count int) {

	res := &testTotalBatchInsertResult{
...
func userTotalBatchInsert(count int, res *testTotalBatchInsertResult) {
	var i int
	userTotals := []*base2.UserTotalBean{}
	for i = 0; i < count; i++ {
		userName := GetRanUserName()
		pingYing, err := base2.GetPingYing(userName)
		if err != nil {
			panic(err)
		}
		...
		userTotal := &base2.UserTotalBean{
			User: user,
			Password: &base2.UserPasswordEntity{
				Password: GetRandStr(6, 20),
			},
			Persona: &base2.UserPersonaBean{
				Name: userName,
				Sex:  int8(base2.RandInt(0, 10) % 3),
			},
			Enterprise: &base2.UserEnterpriseBean{
				Name: "" + GetRandStr(1, 5) + "-有限公司",
				Orgs: []base2.UserEnterpriseOrgBean{
					{
						Name:     "" + GetRandStr(1, 5) + "-机动部门",
						Position: "" + GetRandStr(1, 5) + "-职务",
					},
					{
						Name:     "" + GetRandStr(1, 5) + "-养老部门",
						Position: "" + GetRandStr(1, 5) + "-职务",
					},
				},
			},
	...
	var successUserTotals []*base2.UserTotalBean
	var errUserTotals []*base2.UserTotalBean
	var errs []error
	var err error
	...
	res.successCount += len(successUserTotals)
	res.errorCount += len(errUserTotals)
	if len(errUserTotals) > 0 {
		for _, e := range errs {
			if !base2.IsValidateError(e) {
				panic(e)
			}
		}
