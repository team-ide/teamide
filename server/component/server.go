...
-----END RSA PRIVATE KEY-----`
	...
	filePath := util.BaseDir + "conf/public.pem"
	...
	filePath = util.BaseDir + "conf/private.pem"
	exists, err = base.PathExists(filePath)
	if err != nil {
		panic(err)
	}
...
func RSAEncrypt(origData string) (res string) {
	...
	res, err := base.RSAEncryptByKey(origData, rsa_public_key)
	if err != nil {
...
func RSADecrypt(crypted string) (res string) {
	...
	res, err := base.RSADecryptByKey(crypted, rsa_private_key)
	if err != nil {
