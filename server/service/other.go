...
func GetRandStr(min int, max int) string {
	var len = base.RandInt(min, max)
	var i = 0
	var str = ""
	for i = 0; i < len; i++ {
		str += randChats[base.RandInt(0, randChatsSize*3)%randChatsSize]
...
func GetRanUserName() string {
	str := firstName[base.RandInt(0, firstNameLen*3)%firstNameLen]
	lSize := base.RandInt(0, 6)%2 + 1
	for i := 0; i < lSize; i++ { //随机产生2位或者3位的名
		str += lastName[base.RandInt(0, lastNameLen*3+i)%lastNameLen]
	}
	return str
}
...
func userTotalBatchInsert(count int, res *testTotalBatchInsertResult) {
	var i int
	userTotals := []*base.UserTotalBean{}
	for i = 0; i < count; i++ {
		userName := GetRanUserName()
		pingYing, err := base.GetPingYing(userName)
		if err != nil {
			panic(err)
		}
		...
		userTotal := &base.UserTotalBean{
			...
			Persona: &base.UserPersonaBean{
				Name: userName,
				Sex:  int8(base.RandInt(0, 10) % 3),
			},
			Enterprise: &base.UserEnterpriseBean{
				Name: "" + GetRandStr(1, 5) + "-有限公司",
				Orgs: []base.UserEnterpriseOrgBean{
					{
