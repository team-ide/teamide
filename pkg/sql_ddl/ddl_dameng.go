...
const (
	DaMengCreateTable = `CREATE TABLE {table} (
[{columns}]
	[PRIMARY KEY {primaryKeys}]
)`
	DaMengCreateTableColumn = `{column}[ {type}][ DEFAULT {default}][ NOT NULL]`
)

var (
	DaMengTypeMap = map[string]func(length int, decimal int) string{}
)

func init() {
	DaMengTypeMap["varchar"] = func(length int, decimal int) string {
		return fmt.Sprintf("varchar(%d)", length)
	}
	DaMengTypeMap["text"] = func(length int, decimal int) string {
		return fmt.Sprintf("text")
	}
	DaMengTypeMap["int"] = func(length int, decimal int) string {
		if decimal > 0 {
			return fmt.Sprintf("number(%d, %d)", length, decimal)
		}
		return fmt.Sprintf("number(%d)", length)
	}
	DaMengTypeMap["bigint"] = func(length int, decimal int) string {
		if decimal > 0 {
			return fmt.Sprintf("number(%d, %d)", length, decimal)
		}
		return fmt.Sprintf("number(%d)", length)
	}
	DaMengTypeMap["number"] = func(length int, decimal int) string {
		if decimal > 0 {
			return fmt.Sprintf("number(%d, %d)", length, decimal)
		}
		return fmt.Sprintf("number(%d)", length)
	}
	DaMengTypeMap["datetime"] = func(length int, decimal int) string {
		return fmt.Sprintf("datetime")
	}
}

func ToDatabaseDDLForDaMeng(database string) (sqls []string, err error) {
...
func ToTableDDLForDaMeng(table TableDetailInfo) (sqls []string, err error) {
	...
	if len(table.Columns) > 0 {
		...
		for _, one := range table.Columns {
			...
			var typeFunc = DaMengTypeMap[strings.ToLower(one.Type)]
			if typeFunc == nil {
			...
			columnSql, err = formatSql(DaMengCreateTableColumn, data)
			if err != nil {
				return
			}
			if columnSql == "" {
	...
	sql, err = formatSql(DaMengCreateTable, data)
	if err != nil {
		return
	}
	if sql != "" {
