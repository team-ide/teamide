...
const (
	KingBaseCreateTable = `CREATE TABLE {table} (
[{columns}]
	[PRIMARY KEY {primaryKeys}]
)`
	KingBaseCreateTableColumn = `{column}[ {type}][ DEFAULT {default}][ NOT NULL]`
)

var (
	KingBaseTypeMap = map[string]func(length int, decimal int) string{}
)

func init() {
	KingBaseTypeMap["varchar"] = func(length int, decimal int) string {
		return fmt.Sprintf("varchar(%d)", length)
	}
	KingBaseTypeMap["text"] = func(length int, decimal int) string {
		return fmt.Sprintf("text")
	}
	KingBaseTypeMap["int"] = func(length int, decimal int) string {
		if decimal > 0 {
			return fmt.Sprintf("number(%d, %d)", length, decimal)
		}
		return fmt.Sprintf("number(%d)", length)
	}
	KingBaseTypeMap["bigint"] = func(length int, decimal int) string {
		if decimal > 0 {
			return fmt.Sprintf("number(%d, %d)", length, decimal)
		}
		return fmt.Sprintf("number(%d)", length)
	}
	KingBaseTypeMap["number"] = func(length int, decimal int) string {
		if decimal > 0 {
			return fmt.Sprintf("number(%d, %d)", length, decimal)
		}
		return fmt.Sprintf("number(%d)", length)
	}
	KingBaseTypeMap["datetime"] = func(length int, decimal int) string {
		return fmt.Sprintf("datetime")
	}
}

func ToDatabaseDDLForKingBase(database string) (sqls []string, err error) {

	return
}

func ToTableDDLForKingBase(table TableDetailInfo) (sqls []string, err error) {
	sqls = []string{}
	var columns string
	...
	if len(table.Columns) > 0 {
		...
		for _, one := range table.Columns {
			...
			var typeFunc = KingBaseTypeMap[strings.ToLower(one.Type)]
			if typeFunc == nil {
			...
			columnSql, err = formatSql(KingBaseCreateTableColumn, data)
			if err != nil {
				return
			}
			if columnSql == "" {
	...
	sql, err = formatSql(KingBaseCreateTable, data)
	if err != nil {
		return
	}
	if sql != "" {
