...
type DatabaseOracleDialect struct {
	DatabaseDialect
}

func (this_ *DatabaseOracleDialect) DatabaseDDL(param *GenerateParam, database *DatabaseModel) (sqlList []string, err error) {

	return
}

func (this_ *DatabaseOracleDialect) TableDDL(param *GenerateParam, database string, table *TableModel) (sqlList []string, err error) {
	sqlList = []string{}
	...
	if len(table.ColumnList) > 0 {
		for _, one := range table.ColumnList {
			...
			var c = DatabaseTypeOracle.GetColumnTypeInfo(one.Type)
			if c == nil {
				err = errors.New("Oracle字段类型[" + one.Type + "]未映射!")
				return
			}
...
func (this_ *DatabaseOracleDialect) TableComment(param *GenerateParam, database string, table string, comment string) (sqlList []string) {
...
func (this_ *DatabaseOracleDialect) ColumnComment(param *GenerateParam, database string, table string, column string, comment string) (sqlList []string) {
...
func (this_ *DatabaseOracleDialect) Index(param *GenerateParam, database string, table string, indexName string, indexType string, columns string) (sqlList []string) {
