...
type DatabaseDMDialect struct {
	DatabaseDialect
}

func (this_ *DatabaseDMDialect) DatabaseDDL(param *GenerateParam, database *DatabaseModel) (sqlList []string, err error) {

	return
}

func (this_ *DatabaseDMDialect) TableDDL(param *GenerateParam, database string, table *TableModel) (sqlList []string, err error) {
	sqlList = []string{}
	...
	if len(table.ColumnList) > 0 {
		for _, one := range table.ColumnList {
			...
			var c = DatabaseTypeDM.GetColumnTypeInfo(one.Type)
			if c == nil {
				err = errors.New("DM字段类型[" + one.Type + "]未映射!")
				return
			}
...
func (this_ *DatabaseDMDialect) TableComment(param *GenerateParam, database string, table string, comment string) (sqlList []string) {
...
func (this_ *DatabaseDMDialect) ColumnComment(param *GenerateParam, database string, table string, column string, comment string) (sqlList []string) {
...
func (this_ *DatabaseDMDialect) Index(param *GenerateParam, database string, table string, indexName string, indexType string, columns string) (sqlList []string) {
