...
Zmodem.Session.Receive = class ZmodemReceiveSession extends Zmodem.Session {
    ...
    async _consume_ZDATA_data(subpacket) {
        ...
        await this._on_data_in(subpacket);
        ...
        */
        ...
        if (subpacket.ack_expected() && !subpacket.frame_end()) {
            await this._send_header("ZACK", Zmodem.ENCODELIB.pack_u32_le(this._file_offset));
        }
    }
            ...
            sess._next_header_handler = {
                ZDATA: function on_ZDATA(hdr) {
                    ...
                    this._next_header_handler = {
                        ZEOF: async function on_ZEOF(hdr) {
    ...
    async _skip() {
        ...
        await this._send_header("ZSKIP");

        return ret;
    }

    async _send_ZRINIT() {
        await this._send_header("ZRINIT", ZRINIT_FLAGS);
    }

    async _consume_ZFIN() {
        this._got_ZFIN = true;
        await this._send_header("ZFIN");
    }

    async _consume_ZEOF(header) {
    ...
    async _send_ZRPOS() {
        await this._send_header("ZRPOS", this._file_offset);
    }
    ...
    async _on_data_in(subpacket) {
        await this._Happen("data_in", subpacket);
...
class Transfer {
     ...
     */
    async send(array_like) {
        await this._send(array_like);
     ...
     */
    async end(array_like) {
        var ret = await this._end(array_like || []);
...
Zmodem.Session.Send = class ZmodemSendSession extends Zmodem.Session {
    ...
    async _send_ZSINIT() {
        ...
        await this._send_header_and_data(
            ["ZSINIT", zsinit_flags],
            [0],
            "end_ack"
        );
    ...
    async send_offer(params) {
        ...
        var doer_func = async function () {
            ...
            await sess._send_header_and_data(["ZFILE"], payload_array, "end_ack");
        ...
        return await doer_func();
    }

    async _send_header_and_data(hdr_name_and_args, data_arr, frameend) {
        ...
        await this._sender(bytes_hdr[0]);
    ...
    async _build_and_send_subpacket(bytes_arr, frameend) {
        await this._sender(this._build_subpacket_bytes(bytes_arr, frameend));
    }
    ...
    */
    ...
    async _end_file(bytes_obj) {
        ...
        await this._send_header("ZEOF", this._file_offset);
    ...
    async close() {
        ...
        var ret = new Promise(function (res, rej) {
            sess._next_header_handler = {
                ZFIN: async function () {
                    await sess._sender(OVER_AND_OUT);
                    sess._sent_OO = true;
    ...
    async _send_file_part(bytes_obj, final_packetend) {
        if (!this._sent_ZDATA) {
            await this._send_header("ZDATA", this._file_offset);
        ...
        while (flag) {
            ...
            await this._build_and_send_subpacket(
                chunk,
